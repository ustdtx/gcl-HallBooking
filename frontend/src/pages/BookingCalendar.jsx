...
const BookingCalendar = () => {
  ...
  const isFDBlockingDay = (date) => {
    ...
    return (
      fdBooking &&
      ['Confirmed', 'Pre-Booked', 'Unpaid', 'Review', 'Unavailable'].includes(
  ...
  const isFNorANBlockingFD = (date) => {
    ...
    return (
      (fnBooking && ['Confirmed', 'Pre-Booked', 'Unpaid', 'Review', 'Unavailable'].includes((fnBooking.status || '').toString())) ||
      (anBooking && ['Confirmed', 'Pre-Booked', 'Unpaid', 'Review', 'Unavailable'].includes((anBooking.status || '').toString()))
    );
  };
  ...
  const getShiftStyle = (date, shift) => {
    ...
    if (booking.status === 'Unavailable') {
      // Unavailable should look like Confirmed (Booked)
  ...
  const handleShowConfirmModal = async () => {
    ...
    if (!authData?.member) {
      ...
      // Add redirected=1 to the redirect URL
      const basePath = window.location.pathname + window.location.search;
      const redirectUrl = basePath + (basePath.includes('?') ? '&' : '?') + 'redirected=1';
  ...
  useEffect(() => {
    // Only restore state if redirected=1 is present in the URL
    const redirected = getQueryParam(location.search, 'redirected');
    if (
      redirected === '1' &&
      bookingState &&
      bookingState.hallId === hallId
    ) {
      ...
      setShowResults(!!bookingState.showResults);
      // Auto-fetch bookings if month and year are present
      if (bookingState.selectedMonth && bookingState.selectedYear) {
        // Wait for state to update, then fetch
        setTimeout(() => {
          // Only fetch if not already loaded
          if (!showResults) {
            // Set the states again to ensure correct values
            setSelectedMonth(bookingState.selectedMonth);
            setSelectedYear(bookingState.selectedYear);
            // Call handleSearch with the correct values
            (async () => {
              setLoading(true);
              try {
                const response = await fetch(
                  `${API_BASE}/api/bookings/hall/${hallId}?month=${bookingState.selectedYear}-${bookingState.selectedMonth}`,
                  {
                    method: 'GET',
                  }
                );
