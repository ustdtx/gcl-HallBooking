...
export default function ReservationDetails() {
  ...
  const { hall, shift, status, booking_date, expired_at } = booking;
  ...
  // Check if Pre-Booked booking is expired (handle ISO 8601 format with Z/UTC)
  let isPreBookExpired = false;
  if (status === "Pre-Booked" && expired_at) {
    // expired_at is in ISO 8601 format, e.g. "2025-06-26T15:40:38.000000Z"
    const expiredDate = new Date(expired_at);
    const now = new Date();
    isPreBookExpired = expiredDate.getTime() < now.getTime();
  }
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-8">
          <div className="mb-4 text-left">
            <a
              ...
              onMouseOver={e => (e.currentTarget.style.color = "#d1b97c")}
              onMouseOut={e => (e.currentTarget.style.color = "#BFA465")}
            >
              ← Back
            </a>
          </div>
          ...
          >
            <h2
              className="text-xl text-[#BFA465] font-bold py-4"
            ...
            <div className="p-6">
              <h3
                className="text-lg font-bold text-[#BFA465] mb-1"
                style={{ textAlign: "left" }}
              >
                {hall.name}
              </h3>
              <p className="text-gray-300 mb-4" style={{ textAlign: "left" }}>
              ...
              <div className="border border-[#B18E4E] p-4 rounded">
                <div className="grid grid-cols-3 gap-y-4 gap-x-8 text-sm">
                  <div style={{ textAlign: 'left' }}>
                    ...
                    <div style={{ color: '#CABEA4', marginBottom: '0.5rem' }}>{authData.member.name}</div>
                  </div>
                  <div style={{ textAlign: 'left' }}>
                    <div style={{ color: '#fff', fontWeight: 600 }}>Club Account:</div>
                    <div style={{ color: '#CABEA4', marginBottom: '0.5rem' }}>{authData.member.club_account}</div>
                  </div>
                  <div style={{ textAlign: 'left' }}>
                    <div style={{ color: '#fff', fontWeight: 600 }}>Status:</div>
                    <div style={{ color: '#B18E4E', marginBottom: '0.5rem', fontWeight: 700 }}>{displayStatus}</div>
                  </div>
                  <div style={{ textAlign: 'left' }}>
                    <div style={{ color: '#fff', fontWeight: 600 }}>Total Amount:</div>
                    <div style={{ color: '#CABEA4', marginBottom: '0.5rem' }}>{fullAmount}tk</div>
                  </div>
                  <div style={{ textAlign: 'left' }}>
                    <div style={{ color: '#fff', fontWeight: 600 }}>Pre Booking Amount:</div>
                    <div style={{ color: '#CABEA4', marginBottom: '0.5rem' }}>{prebookAmount}tk</div>
                  </div>
                  <div style={{ textAlign: 'left' }}>
                    <div style={{ color: '#fff', fontWeight: 600 }}>Amount to be Paid:</div>
                    <div style={{ color: '#CABEA4', marginBottom: '0.5rem' }}>{remaining === null ? "NULL" : `${remaining.toLocaleString()}tk`}</div>
                  </div>
                  <div style={{ textAlign: 'left' }}>
                    <div style={{ color: '#fff', fontWeight: 600 }}>Date:</div>
                    <div style={{ color: '#CABEA4', marginBottom: '0.5rem' }}>{new Date(booking_date).toLocaleDateString()}</div>
                  </div>
                  <div style={{ textAlign: 'left' }}>
                    <div style={{ color: '#fff', fontWeight: 600 }}>Shift:</div>
                    <div style={{ color: '#CABEA4', marginBottom: '0.5rem' }}>{shiftLabel}</div>
                  </div>
                </div>
              </div>
                ...
                <div className="flex flex-row gap-3 w-full justify-center">
                  ...
                  <button
                    ...
                    disabled={
                      ...
                      status === "Review" ||
                      (status === "Pre-Booked" && isPreBookExpired)
                    }
                    style={{
